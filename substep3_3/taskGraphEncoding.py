...
def hungarian_matching(visual_embs: torch.Tensor,
    ...
    v = torch.nn.functional.normalize(visual_embs, p=2, dim=1)
    t = torch.nn.functional.normalize(text_embs,   p=2, dim=1)
    sim_matrix = (v @ t.T).cpu().numpy()          # (N, M)
    ...
    return visual_indices, node_indices, similarities
