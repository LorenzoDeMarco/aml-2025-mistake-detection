...
def train_loo_fold(fold_id, test_video_id, train_ids, test_ids, global_visual, global_text, args):
    ...
    wandb.init(
        ...
        name=f"LOO_Video_{test_video_id}",
        config=args,
        reinit=True,
        mode="online"
    )
...
if __name__ == "__main__":
    ...
    progress_records = []
    print(f"\n=======================================================================")
    print(f" Starting Leave-One-Out (LOO) across {len(all_video_ids)} videos.")
    print(f"=======================================================================\n")

    for fold, test_video_id in enumerate(all_video_ids):
        fold_start_time = time.time()

        # In LOO, test set is exactly one video
        test_ids = [test_video_id]
        train_ids = [vid for vid in all_video_ids if vid != test_video_id]

        print(f"\n[{fold+1}/{len(all_video_ids)}] Start Fold | Testing unseen video: '{test_video_id}'")

        results = train_loo_fold(fold, test_video_id, train_ids, test_ids, global_visual, global_text, hyperparameters)

        print(f"    [Valutazione FOLD completata - Risultato sul video unseen]:")
        for res in results:
    ...
    print("\n================ FINAL GLOBAL GNN PERFORMANCE EVALUATION (LOO) ================")
