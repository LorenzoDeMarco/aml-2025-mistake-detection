...
class NodeFusion(nn.Module):
    ...
    def forward(self, x_text, x_vis, x_sim):
        if self.training:
            x_text = self.dropout(x_text)
            has_visual = (x_vis.norm(dim=-1, keepdim=True) > 0).float()
            x_vis = self.dropout(x_vis * has_visual)

        ht = self.text_proj(x_text)   # (V, hidden)
        hv = self.vis_proj(x_vis)     # (V, hidden)
...
class DAGNNLayer(nn.Module):
    ...
    def forward(self, x, edge_index, node_levels):
        ...
        for level in range(max_level + 1):
            ...
            h_batch_new = self.gru(msgs_batch, h_batch_prev)
...
class GraphClassifier(nn.Module):
    def __init__(
        ...
        self.fusion = NodeFusion(dim_text, dim_vis, hidden_dim, dropout)
...
def load_recordings_combined(recordings_json: Path, label_rule: str = "any_step_error") -> Tuple[Dict[str, int], Dict[str, List[str]]]:
    ...
    for vid, item in db.items():
        ...
        if label_rule == "any_step_error":
            labels[str(vid)] = 1 if any_step_error else 0
        elif label_rule == "video_has_error":
            labels[str(vid)] = 1 if bool(item.get("has_error", False)) else 0
        else:
...
def print_model_metrics(fold, oof_preds, oof_labels, oof_probs):
    ...
    tp = int(((np_pred == 1) & (np_true == 1)).sum())
    tn = int(((np_pred == 0) & (np_true == 0)).sum())
    fp = int(((np_pred == 1) & (np_true == 0)).sum())
    fn = int(((np_pred == 0) & (np_true == 1)).sum())
...
def get_probs(model: nn.Module, loader, device: torch.device, compute_preds: bool = True, threshold: float = 0.5):
    """Returns (all_probs, all_labels, all_logits, preds) arrays from the given loader.

    If `compute_preds` is False, `preds` in the return tuple will be `None`.
    """
    model.eval()
    ...
    preds = (all_probs >= threshold).astype(int) if compute_preds else None
    ...
    for fold, (train_idx, val_idx) in enumerate(splitter.split(indices, groups=groups), 1):
        ...
        for ep in range(1, args.epochs + 1):
            tr_loss = train_one_epoch(
                ...
                args.grad_clip, pos_weight=pos_weight, label_smoothing=args.label_smoothing
            )
...
def main():
    ...
    if args.eval_only:
        use_threshold = 0.5
        if val_loader:
            val_probs, val_labels, _, _ = get_probs(model, val_loader, device, threshold=0.5)
            use_threshold = find_optimal_threshold(val_probs, val_labels)
            val_preds = (val_probs >= use_threshold).astype(int)
            print(f"[VAL] Threshold selected from validation: {use_threshold:.4f}")
            print_model_metrics("VAL_OPTIMAL", val_preds, val_labels, val_probs)
            compute_detailed_metrics(val_labels, val_preds)

        if test_loader:
            test_probs, test_labels, _, _ = get_probs(model, test_loader, device, threshold=0.5)
            test_preds = (test_probs >= use_threshold).astype(int)
            print(f"[TEST] Threshold applied: {use_threshold:.4f}")
            print_model_metrics("TEST_OPTIMAL", test_preds, test_labels, test_probs)
    ...
    for epoch in range(1, args.epochs + 1):
        tr_loss = train_one_epoch(model, train_loader, device, optimizer, args.grad_clip, label_smoothing=args.label_smoothing)
        scheduler.step()
    ...
    if test_loader:
        ...
        test_probs, test_labels, _, _ = get_probs(model, test_loader, device, threshold=0.5)
        test_preds = (test_probs >= use_threshold).astype(int)
        print(f"[FINAL TEST] Threshold applied: {use_threshold:.4f}")
        print_model_metrics("FINAL_OPTIMAL", test_preds, test_labels, test_probs)
        compute_detailed_metrics(test_labels, test_preds)
        if use_wandb:
