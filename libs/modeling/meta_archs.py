            ...
            for i, lbl in enumerate(gt_labels):
                ...
                if torch.is_tensor(lbl):
                    if lbl.numel() == 0:
                        ...
                        print(f"[{i}] labels min/max: {lbl.min().item()} {lbl.max().item()} shape={tuple(lbl.shape)}")
                else:
                    # Python list / numpy array
                    if len(lbl) == 0:
                        print(f"[{i}] labels=empty list")
                    else:
                        print(f"[{i}] labels min/max: {min(lbl)} {max(lbl)} len={len(lbl)}")
